var More = {
    ...
    showMenu: function () {
        ...
        container.innerHTML =
            ...
            '<div class="more-text"><span class="more-label">Анализы, обследования, ЭКГ</span><span class="more-desc">Результаты и заключения</span></div>' +
            '</button>' +
    ...
    renderAnalyses: function (container) {
        ...
        var html = '<div class="section-header">' +
            '<button class="btn btn-outline btn-back" onclick="More.showMenu()">← Назад</button>' +
            '<h2>Анализы, обследования, ЭКГ</h2>' +
            '</div>';
    ...
    renderAbout: function (container) {
        ...
        html += '<div class="about-card">' +
            ...
            'пульс, сатурация (SpO2), гликемия натощак и после еды, температура, вес</li>' +
            '<li><strong>Проверок при вводе:</strong> 18</li>' +
            '<li><strong>Нормы показателей:</strong> 6 возрастных категорий и 15 статей ' +
            'по диагнозам; подбираются по карточке профиля, индекс массы тела ' +
            'считается автоматически</li>' +
            '<li><strong>Отметки отклонений:</strong> 🟡 в пределах шага тревоги, ' +
            '🔴 дальше шага</li>' +
            '<li><strong>Измерений в одном дне:</strong> до 36</li>' +
            '<li><strong>Графики:</strong> сразу по нескольким показателям, ' +
            'точки окрашены по отклонению от нормы</li>' +
        ...
        html += '<div class="about-card">' +
            ...
            'Дата рождения, рост и диагнозы обязательны: по ним подбираются нормы ' +
            'показателей и считается индекс массы тела. ' +
            ...
            'в чате. Кнопка «Создать график» построит график по отмеченным дням: ' +
            ...
            'Кнопка «❤️ Запись ЭКГ с домашнего электрокардиографа» открывает раздел ' +
            '«Анализы, обследования, ЭКГ»: снимок с домашнего кардиографа сохраняется ' +
            'там же и оттуда отправляется доктору на расшифровку. ' +
            ...
            '(PDF, фото) через кнопку 📎 или перетащите их мышью в область переписки. ' +
            'Доктор расшифрует результаты и даст рекомендации. ' +
            ...
            '<div class="about-step"><span class="about-step-icon">📄</span>' +
            '<div><strong>Анализы, обследования, ЭКГ</strong> — храните результаты обследований ' +
            'и записи ЭКГ с домашнего электрокардиографа с прикреплёнными PDF-файлами. ' +
        ...
        html += '<div class="about-card">' +
            ...
            '<li>Напоминание о резервной копии данных</li>' +
            '<li>Несколько членов семьи: у каждого свои дневник, консультации и графики</li>' +
            '<li>Индивидуальные нормы по возрасту и диагнозам, отклонения в процентах</li>' +
            '<li>Автоматический расчёт индекса массы тела</li></ul></div>' +

            '<div class="about-release">' +
            '<div class="about-release-head"><span class="about-release-ver">v3.2</span>' +
            '<span class="about-release-date">август 2026</span>' +
            '<span class="about-release-status about-done">запущено</span></div>' +
            '<ul><li>Гипертония разделена на три степени, диабет — на три типа</li>' +
            '<li>В группе можно отметить только один диагноз: остальные пункты гаснут</li>' +
            '<li>Границы норм расширены, чтобы не создавать ложных тревог</li>' +
            '<li>У каждой статьи норм появилось примечание о консультации врача</li>' +
            '<li>Отметки отклонений: жёлтая от 5 до 20%, красная свыше 20%</li>' +
            '<li>При нескольких диагнозах границы объединяются по каждому показателю</li>' +
            '<li>Исправлен подсчёт выходов за норму в статистике за период</li>' +
            '<li>Кнопка «Завершить запись» больше не открывает печать</li>' +
            '<li>Печать периода переведена на браузерную печать — ' +
            'документ не обрезается на смартфоне</li></ul></div>' +

            '<div class="about-release">' +
            '<div class="about-release-head"><span class="about-release-ver">v3.3</span>' +
            '<span class="about-release-date">август 2026</span>' +
            '<span class="about-release-status about-done">запущено</span></div>' +
            '<ul><li>Трёхуровневый контроль показателей</li>' +
            '<li>Уровень 1 — тревога сразу при вводе: 11 порогов, одинаковых ' +
            'для всех и не зависящих от таблицы норм</li>' +
            '<li>Уровень 2 — итог за день по каждому показателю вместо перечня ' +
            'каждого измерения</li>' +
            '<li>Уровень 3 — график и статистика за период, как прежде</li>' +
            '<li>Вместо процента отклонения — «шаг тревоги» в единицах ' +
            'показателя: «на 11 ниже границы 90»</li>' +
            '<li>Диагнозы теперь видны в списке членов семьи</li>' +
            '<li>Кнопка «Удалить» появляется сразу под новым ответом доктора</li>' +
            '<li>Таблица в документе с графиком больше не рвётся между ' +
            'страницами</li>' +
            '<li>Приложение стало легче на полмегабайта: три библиотеки ' +
            'печати больше не нужны</li></ul></div>' +
            ...
            '<li>Если в семье несколько человек, приложение спрашивает, ' +
            'кого консультируем, и переключает переписку</li>' +
            '<li>Новый диагноз: гипотензия (хроническое низкое давление)</li>' +
            '<li>В сводке за день количество измерений и номер измерения ' +
            'больше не путаются</li>' +
            ...
            '<li>Показателей для графиков можно выбрать сразу несколько</li>' +
            '<li>Графики строятся один под другим, в одном документе</li>' +
            '<li>Точки на графике окрашены: белая — норма, жёлтая — ' +
            'внимание, красная — критично</li>' +
            ...
            'не получает красную отметку</li>' +
            '<li>Порог критики свой у каждой категории: у здоровых 2.8, ' +
            'у пожилых 3.0, при завышенных целевых уровнях 3.5</li>' +
            ...
            '<li>Размер точек на графике подбирается по их числу: на длинном ' +
            'периоде отметки мельче и больше не сливаются в цепочку</li>' +
            '<li>Первое измерение дня отмечено квадратом, остальные — ' +
            ...
            'минуя меню «Ещё»</li>' +
            '<li>Раздел «Анализы и обследования» переименован в ' +
            '«Анализы, обследования, ЭКГ» — на главной, в меню «Ещё» ' +
