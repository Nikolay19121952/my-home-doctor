...
var SYSTEM_PROMPT = [
    ...
    '6. Не спрашивай имя, фамилию, адрес, телефон и другие сведения, по которым человека можно опознать. Приложение их не передаёт, и для консультации они не нужны. Обращайся к собеседнику без имени.',
    '',
    'КАК ВЕСТИ ДИАЛОГ:',
    ...
    '',
    'ДЛИННЫЕ ОТВЕТЫ:',
    'Если ответ получается очень длинным (подробная расшифровка анализов, полное медицинское заключение с таблицами), раздели его на логические части (например, одно исследование = одна часть).',
    'В конце каждой незавершённой части поставь ТОЛЬКО маркер [ПРОДОЛЖЕНИЕ] на отдельной строке. Больше ничего не добавляй после маркера.',
    'ЗАПРЕЩЕНО писать в ответе: «Часть X из Y», «Напишите Продолжай», «Продолжение следует», «[КНОПКА ПЕЧАТЬ]», «[КНОПКА ЭКСПОРТ В PDF]» и любые другие служебные/навигационные тексты.',
    'Система автоматически запросит продолжение и склеит все части в один документ. Пиши только содержательный медицинский текст.'
].join('\n');
    ...
    var apiReq = https.request(options, function (apiRes) {
        ...
        apiRes.on('end', function () {
            try {
                ...
                // Берём первый текстовый блок, а не первый попавшийся:
                // модель может вернуть перед ним блок с размышлениями
                var reply = '';
                var blocks = parsed.content || [];
                for (var b = 0; b < blocks.length; b++) {
                    if (blocks[b] && blocks[b].type === 'text' && blocks[b].text) {
                        reply = blocks[b].text;
                        break;
                    }
                }
